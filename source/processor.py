    ...
    def convert_examples_to_features(config: Config, examples, tokenizer):
        ...
        for ex_index, example in enumerate(tqdm(examples, desc="convert examples")):
            ...
            for i, word in enumerate(example_text_list):
                ...
                # 如果tokenized_word长度大于1，表示分词器将一个词分成了多个部分
                if len(tokenized_word) > 1:
                    # 为每个分词后的部分分配相同的标签
                    labels.extend([example_label_list[i]] * len(tokenized_word))
                    
                    # 进一步检查分词结果是否真的不符合预期
                    if word.isalnum() and len(tokenized_word) > 1:
                        word_piece = True  # 发现真正的问题单词
                else:
                    # 否则，只添加一个标签
                    labels.append(example_label_list[i])

            # 仅当存在 word piece 时记录日志
            if word_piece:
